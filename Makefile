...
strict: EXTRA_CFLAGS += -O0 -g3 ${SAN_FLAGS}
strict: EXTRA_LDFLAGS += ${SAN_FLAGS}
strict: WERROR=1
strict: clean cupidwm cupidwmctl
...
build/cupidwmctl.o: ${CTL_SRC} | build
	${CC} -c ${CFLAGS} ${CTL_SRC} -o build/cupidwmctl.o
...
check: debug lint test-packaging
	@if [ -n "$$DISPLAY" ] && \
...
dist: clean
	...
	cp -R LICENSE LICENSES THIRD_PARTY_NOTICE.md SECURITY.md CHANGELOG.md \
		...
		src scripts tests tools docs img examples completions packaging undefined-medium.ttf cupidwm-${VERSION}
...
distcheck: dist
	...
	test -f "$$tmpdir/cupidwm-${VERSION}/docs/ecosystem.md"; \
	...
	test -f "$$tmpdir/cupidwm-${VERSION}/tests/packaging/sanity.sh"; \
	test -f "$$tmpdir/cupidwm-${VERSION}/completions/cupidwmctl.bash"; \
	test -f "$$tmpdir/cupidwm-${VERSION}/examples/xinitrc"; \
	test -f "$$tmpdir/cupidwm-${VERSION}/packaging/arch/PKGBUILD"; \
